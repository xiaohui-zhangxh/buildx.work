var e={};function deepFreeze(e){e instanceof Map?e.clear=e.delete=e.set=function(){throw new Error("map is read-only")}:e instanceof Set&&(e.add=e.clear=e.delete=function(){throw new Error("set is read-only")});Object.freeze(e);Object.getOwnPropertyNames(e).forEach((t=>{const n=e[t];const i=typeof n;i!=="object"&&i!=="function"||Object.isFrozen(n)||deepFreeze(n)}));return e}
...
/**
   * @param {Renderer} builder
   * @param {Node} node
   */static _walk(e,t){if(typeof t==="string")e.addText(t);else if(t.children){e.openNode(t);t.children.forEach((t=>this._walk(e,t)));e.closeNode(t)}return e}
/**
   * @param {Node} node
 ...
 * @returns {object}
 */function stripOptionsFromArgs(e){const t=e[e.length-1];if(typeof t==="object"&&t.constructor===Object){e.splice(e.length-1,1);return t}return{}}
 ...
 * @param {RegExp} re
 * @param {string} lexeme
 */function startsWith(e,t){const n=e&&e.exec(t);return n&&n.index===0}const n=/\[(?:[^\\\]]|\\.)*\]|\(\??|\\([1-9][0-9]*)|\\./;
/**
 ...
 *
 * @type {CompilerExt}
 */function scopeClassName(e,t){if(e.className!==void 0){e.scope=e.className;delete e.className}}
/**
 * `beginKeywords` syntactic sugar
 * @type {CompilerExt}
 */function beginKeywords(e,t){if(t&&e.beginKeywords){e.begin="\\b("+e.beginKeywords.split(" ").join("|")+")(?!\\.)(?=\\b|\\s)";e.__beforeBegin=skipIfHasPrecedingDot;e.keywords=e.keywords||e.beginKeywords;delete e.beginKeywords;e.relevance===void 0&&(e.relevance=0)}}
/**
...
const i=Object.create(null);typeof e==="string"?compileList(n,e.split(" ")):Array.isArray(e)?compileList(n,e):Object.keys(e).forEach((function(n){Object.assign(i,compileKeywords(e[n],t,n))}));return i;
/**
...
/**
 * @param {CompiledMode} mode
 */function beginMultiClass(e){if(Array.isArray(e.begin)){if(e.skip||e.excludeBegin||e.returnBegin){error("skip, excludeBegin, returnBegin not compatible with beginScope: {}");throw L}if(typeof e.beginScope!=="object"||e.beginScope===null){error("beginScope must be object");throw L}remapScopeNames(e,e.begin,{key:"beginScope"});e.begin=_rewriteBackreferences(e.begin,{joinWith:""})}}
/**
 * @param {CompiledMode} mode
 */function endMultiClass(e){if(Array.isArray(e.end)){if(e.skip||e.excludeEnd||e.returnEnd){error("skip, excludeEnd, returnEnd not compatible with endScope: {}");throw L}if(typeof e.endScope!=="object"||e.endScope===null){error("endScope must be object");throw L}remapScopeNames(e,e.end,{key:"endScope"});e.end=_rewriteBackreferences(e.end,{joinWith:""})}}
/**
...
/**
   ...
   */
function langRe(t,n){return new RegExp(source(t),"m"+(e.case_insensitive?"i":"")+(e.unicodeRegex?"u":"")+(n?"g":""))}class MultiRegex{constructor(){this.matchIndexes={};this.regexes=[];this.matchAt=1;this.position=0}addRule(e,t){t.position=this.position++;this.matchIndexes[this.matchAt]=t;this.regexes.push([t,e]);this.matchAt+=countMatchGroups(e)+1}compile(){this.regexes.length===0&&(this.exec=()=>null);const e=this.regexes.map((e=>e[1]));this.matcherRe=langRe(_rewriteBackreferences(e,{joinWith:"|"}),true);this.lastIndex=0}
/** @param {string} s */exec(e){this.matcherRe.lastIndex=this.lastIndex;const t=this.matcherRe.exec(e);if(!t)return null;const n=t.findIndex(((e,t)=>t>0&&e!==void 0));const i=this.matchIndexes[n];t.splice(0,n);return Object.assign(t,i)}}class ResumableMultiRegex{constructor(){this.rules=[];this.multiRegexes=[];this.count=0;this.lastIndex=0;this.regexIndex=0}getMatcher(e){if(this.multiRegexes[e])return this.multiRegexes[e];const t=new MultiRegex;this.rules.slice(e).forEach((([e,n])=>t.addRule(e,n)));t.compile();this.multiRegexes[e]=t;return t}resumingScanAtSamePosition(){return this.regexIndex!==0}considerAll(){this.regexIndex=0}addRule(e,t){this.rules.push([e,t]);t.type==="begin"&&this.count++}
 ...
 * @param {Mode} mode
 * @returns {Mode | Mode[]}
 * */function expandOrCloneMode(e){e.variants&&!e.cachedVariants&&(e.cachedVariants=e.variants.map((function(t){return inherit$1(e,{variants:null},t)})));return e.cachedVariants?e.cachedVariants:dependencyOnParent(e)?inherit$1(e,{starts:e.starts?inherit$1(e.starts):null}):Object.isFrozen(e)?inherit$1(e):e}var S="11.11.1";class HTMLInjectionError extends Error{constructor(e,t){super(e);this.name="HTMLInjectionError";this.html=t}}
/**
@typedef {import('highlight.js').Mode} Mode
@typedef {import('highlight.js').CompiledMode} CompiledMode
...
/**
  ...
  */function highlight(e,t,n){let i="";let o="";if(typeof t==="object"){i=e;n=t.ignoreIllegals;o=t.language}else{deprecated("10.7.0","highlight(lang, code, ...args) has been deprecated.");deprecated("10.7.0","Please use highlight(code, options) instead.\nhttps://github.com/highlightjs/highlight.js/issues/2277");o=e;i=t}n===void 0&&(n=true)
...
/** @type HighlightResult */let e=null;if(typeof d.subLanguage==="string"){if(!t[d.subLanguage]){p.addText(m);return}e=_highlight(d.subLanguage,m,true,f[d.subLanguage]);f[d.subLanguage]=/** @type {CompiledMode} */e._top}else e=highlightAuto(m,d.subLanguage.length?d.subLanguage:null);d.relevance>0&&(b+=e.relevance);p.__addSublanguage(e._emitter,e.language)}function processBuffer(){d.subLanguage!=null?processSubLanguage():processKeywords();m=""}
/**
     * @param {string} text
     * @param {string} scope
...
/**
     ...
     * @param {RegExpMatchArray} match
     */function startNewMode(e,t){e.scope&&typeof e.scope==="string"&&p.openNode(g.classNameAliases[e.scope]||e.scope);if(e.beginScope)if(e.beginScope._wrap){emitKeyword(m,g.classNameAliases[e.beginScope._wrap]||e.beginScope._wrap);m=""}else if(e.beginScope._multi){emitMultiClass(e.beginScope,t);m=""}d=Object.create(e,{parent:{value:d}});return d}
/**
...
/**
     ...
     */function doIgnore(e){if(d.matcher.regexIndex===0){m+=e[0];return 1}x=true;return 0}
/**
...
/**
     ...
     */function processLexeme(t,s){const r=s&&s[0];m+=t;if(r==null){processBuffer();return 0}if(l.type==="begin"&&s.type==="end"&&l.index===s.index&&r===""){m+=n.slice(s.index,s.index+1);if(!o){
/** @type {AnnotatedError} */
const t=new Error(`0 width match regex (${e})`);t.languageName=e;t.badRule=l.rule;throw t}return 1}l=s;if(s.type==="begin")return doBeginMatch(s);if(s.type==="illegal"&&!i){
/** @type {AnnotatedError} */
const e=new Error('Illegal lexeme "'+r+'" for mode "'+(d.scope||"<unnamed>")+'"');e.mode=d;throw e}if(s.type==="end"){const e=doEndMatch(s);if(e!==v)return e}if(s.type==="illegal"&&r===""){m+="\n";return 1}if(E>1e5&&E>s.index*3){const e=new Error("potential infinite loop, way more iterations than matches");throw e}m+=r;return r.length}const g=getLanguage(e);if(!g){error(s.replace("{}",e));throw new Error('Unknown language: "'+e+'"')}const u=compileLanguage(g);let h="";
...
/**
   ...
   */function configure(e){a=T(a,e)}const initHighlighting=()=>{highlightAll();deprecated("10.6.0","initHighlighting() deprecated.  Use highlightAll() now.")};function initHighlightingOnLoad(){highlightAll();deprecated("10.6.0","initHighlightingOnLoad() deprecated.  Use highlightAll() now.")}let c=false;function highlightAll(){function boot(){highlightAll()}if(document.readyState==="loading"){c||window.addEventListener("DOMContentLoaded",boot,false);c=true;return}const e=document.querySelectorAll(a.cssSelector);e.forEach(highlightElement)}
/**
...
/**
   ...
   */function registerAliases(e,{languageName:t}){typeof e==="string"&&(e=[e]);e.forEach((e=>{n[e.toLowerCase()]=t}))}
/**
...
/**
   * @param {HLJSPlugin} plugin
   */function removePlugin(e){const t=i.indexOf(e);t!==-1&&i.splice(t,1)}
/**
   *
