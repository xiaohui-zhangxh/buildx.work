...
- 🔐 **认证与授权**：✅ 邮箱登录、密码找回、会话管理、个人中心 | ⏳ 手机号登录、OAuth、2FA
- 👥 **用户与角色**：✅ 用户管理、RBAC、权限控制、批量操作、管理后台 | ⏳ 数据导入导出
- 🏢 **多租户**：⏳ 租户隔离、Account 模型（外部 ID、Slug、JoinCode）、组织架构、团队协作
- 💬 **内容交互系统**：⏳ 评论系统（多态关联）、提及系统、反应系统（多态关联）
- 🏷️ **内容组织系统**：⏳ 标签系统（多态关联）、置顶系统（多态关联）、关注系统（多态关联）、分配系统（多态关联）
- 🔍 **搜索与筛选**：⏳ 全文搜索（自动索引）、高级筛选（条件保存）
- 🔐 **访问控制**：⏳ 资源访问控制（多态关联、访问级别、访问历史）
- 📊 **事件系统**：⏳ Event 审核日志（多态关联、Eventable Concern、Webhook 分发）
- 📡 **Webhook**：⏳ 事件订阅、签名验证、重试机制
- 🔔 **通知系统**：✅ 邮件通知（用户确认、密码重置） | ⏳ 站内通知（多态关联、已读/未读、推送通知）、通知聚合、短信通知
...
1. **第一阶段**：用户认证系统 ✅ **已完成**
   ...
   - 密码过期检查和提醒
2. **第二阶段**：权限系统 + 管理后台 ✅ **已完成**
   - 基于 Action Policy 的权限系统
   - RBAC 角色管理系统
   - 管理后台（用户管理、角色管理、系统配置、操作日志、仪表盘）
   - 搜索、筛选、批量操作功能
3. **第三阶段**：多租户支持 📋 **计划中**
   - Account 模型（SaaS 租户账号）
   - 外部 ID、Slug、JoinCode 机制
   - 数据隔离和组织架构
4. **第四阶段**：通用业务功能系统 📋 **计划中**
   - 内容交互系统（评论、提及、反应）
   - 内容组织系统（标签、置顶、关注、分配）
   ...
   - 访问控制系统
   - 事件系统（Event 审核日志）
5. **第五阶段**：通知与 Webhook 系统 📋 **计划中**
   - 站内通知系统（多态关联、推送通知、通知聚合）
   - Webhook 系统
6. **第六阶段**：高级功能 📋 **计划中**
   - API 支持
   - AI MCP 服务集成

📖 [查看详细开发计划](../../docs/DEVELOPMENT_PLAN.md) | [阶段文档](../../docs/README.md) | [Fizzy 最佳实践学习](../../docs/experiences/fizzy-overview.md)
