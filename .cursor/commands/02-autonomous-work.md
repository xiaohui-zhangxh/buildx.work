...
当用户说"我去休息了，接下来的工作就完全托管给你"或类似表达时，进入自主工作模式。AI 完全自主工作，持续完成任务直到完成。

**⚠️ 重要**：进入自主工作模式后，AI **必须先制定计划并等待用户确认**，**禁止直接开始执行**。

## 🎯 核心原则

1. **完全自主**：不需要用户确认，持续工作直到完成
2. **按计划执行**：按照既定的开发计划（如 `docs/phase-*/plan.md`）执行任务
3. **测试驱动**：每个功能都要编写测试，确保所有测试通过
4. **持续工作**：完成一个任务后立即开始下一个，不中断
...
- **不要等待**：完成一个任务后立即开始下一个
- **不要停下来报告**：不要在每个任务完成后停下来向用户报告，继续执行下一个任务
- **只有全部完成才报告**：只有当所有计划任务都完成后，才向用户报告完成情况
- **持续执行**：确认后，AI 应该像一个连续运行的脚本一样，持续执行直到所有任务完成
