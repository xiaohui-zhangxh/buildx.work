...
> 帮助用户识别可以贡献给基础平台的代码，并协助提取和贡献

## 📋 指令说明

此指令帮助用户识别当前项目中可以贡献给 BuildX.work 基础平台的代码，并协助提取、通用化处理和贡献。

**重要**：此指令适用于 fork 项目（业务项目），用于识别和贡献通用功能到基础平台。
...
1. **必须使用 GitHub Pull Request**：所有贡献必须通过 GitHub Pull Request 提交，不要使用其他方式（如 Git 补丁、直接提交等）
2. **遵循 GitHub 最佳实践**：
   - 使用清晰的分支命名
   - 使用规范的提交信息
   - 提供详细的 PR 描述
   - 响应审查意见
   - 保持分支更新
3. **一个 PR 一个功能**：每个 PR 只包含一个功能或修复，避免混合多个不相关的更改
4. **保持 PR 简洁**：如果功能很大，考虑拆分为多个 PR
5. **提供完整信息**：PR 描述要包含功能描述、使用场景、测试结果、向后兼容性等信息
