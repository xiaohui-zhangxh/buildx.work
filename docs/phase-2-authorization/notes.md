...
**与阿里云 RAM 的对比**：
- **阿里云 RAM**：使用 JSON 格式的策略定义，存储在数据库中，适合配置化的权限管理
- **Action Policy**：使用 Ruby Policy 类定义权限规则，代码即配置，适合应用内部权限管理

**我们的方案**：
- 使用 **Action Policy** 的 Policy 类来定义权限规则（代码中定义）
- 使用 **Role 模型**来管理角色（数据库中存储）
- 在 Policy 类中通过角色判断权限（如：`user.has_role?(:admin)`）
- 支持资源级权限控制（在 Policy 类中实现）

**优势**：
1. 权限规则在代码中，易于版本控制和代码审查
2. 支持复杂的业务逻辑（如：只能编辑自己的资源）
3. 高性能（Action Policy 有缓存机制）
4. 易于测试（每个 Policy 类都可以独立测试）

### 权限系统设计参考：阿里云 RAM

#### 阿里云 RAM 的核心设计

阿里云 RAM（Resource Access Management）采用基于策略的权限模型，主要包含以下核心概念：
...
2. **角色（RAM角色）**
   - 一种身份，可以被"扮演"（AssumeRole）
   - 通过角色扮演获取临时凭证（STS Token）
   - 支持跨账号访问和临时权限授予
